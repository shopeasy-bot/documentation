...
export function LLMCopyButton({
  ...
  return (
    ...
    >
      {checked ? <Check /> : <Copy />}
      {checked ? "Copiado!" : "Copiar Markdown"}
    </button>
  );
}

const optionVariants = cva(
  "text-sm p-2 rounded-lg inline-flex items-center gap-2 " +
  ...
  className={cn(
    buttonVariants({
      ...
      className:
        ...
        "hover:bg-fd-accent/30 hover:shadow " +
        "active:scale-[0.97]",
    })
  )}
>
  <Bot className="size-4 text-fd-muted-foreground transition-colors group-hover:text-fd-foreground" />

  <span className="font-medium leading-none">
    Perguntar à IA
  </span>

  <ChevronDown className="size-3.5 text-fd-muted-foreground transition-transform group-hover:translate-y-[1px]" />
</PopoverTrigger>
